...
|--------------------------------------------------------------------------
| Lưu ý: cần RoleMiddleware hỗ trợ nhiều role (xem bên dưới).
*/
Route::prefix('staff')
    ->name('staff.')
    ...
    ->group(function () {
        ...
        Route::prefix('orders')->name('orders.')->group(function () {
            ...
            Route::patch('/{order}/update-status',   [ OrderController::class, 'updateStatus'])->name('updateStatus');
            Route::patch('/{order}/update-shipping', [ OrderController::class, 'updateShippingInfo'])->name('updateShippingInfo');
            Route::post('/{order}/ready-to-ship',    [ OrderController::class, 'readyToShip'])->name('readyToShip');
            Route::patch('/{order}/cod-paid',        [ OrderController::class, 'markCodPaid'])->name('codPaid');
        });

        // ===== REVIEWS =====
        Route::prefix('reviews')->name('reviews.')->group(function () {
            Route::get('/',                  [ ProductReviewController::class, 'index'])->name('index');
            Route::post('/{review}/toggle-status', [ ProductReviewController::class, 'toggleStatus'])->name('toggleStatus');
            Route::delete('/{review}',       [ ProductReviewController::class, 'destroy'])->name('destroy');
        });

        // ===== PRODUCTS / CATEGORIES / BRANDS / SUPPLIERS / INVENTORIES / VOUCHERS / SLIDES / BLOGS =====
        Route::resource('products',     ProductController::class);
        Route::get('/products/{product}/variants-inventory', [ ProductController::class, 'getVariantsWithInventory'])
            ->name('products.variants.inventory');
        ...
        // (Tùy ý) Cấm staff vào settings/users… bằng cách KHÔNG khai báo route staff cho nhóm đó.
    });
