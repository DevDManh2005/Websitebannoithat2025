...
<head>
  ...
  <style>
    ...
    .is-home .header-home,
    .is-home .header-home .navbar{ background:transparent!important; box-shadow:none!important; border:0!important; }
    .is-home .header-home.is-scrolled{ background:rgba(0,0,0,.45)!important; backdrop-filter:blur(6px) saturate(1.1); }

    /* ===== Mega menu (catmega) – canh giữa theo trigger ===== */
    .navbar{ position:relative; }               /* toạ độ tuyệt đối của menu bám theo navbar */
    .navbar *{ overflow:visible!important; }    /* tránh ancestor cắt menu */
    .nav-item.dropdown.mega{ position:static!important; } /* để .dropdown-menu có thể thoát khỏi li */
    ...
    .dropdown-menu.catmega{
      ...
      box-shadow:0 18px 48px rgba(2,6,23,.20); z-index:1060; overflow:clip;
    }
    /* Vô hiệu hoá Popper của Bootstrap với menu này */
    .dropdown-menu.catmega[data-bs-popper]{ left:0!important; top:auto!important; transform:none!important; margin:0!important; }
    .dropdown-menu.catmega.show{ opacity:1; visibility:visible; pointer-events:auto; transition:opacity .18s ease; }

    /* Lưới cột */
    .catmega-inner{ padding:16px 18px; }
    .catmega-grid{ display:grid; grid-template-columns:repeat(4,1fr); gap:16px 24px; }
    .catmega-col{ min-width:0; border-right:1px solid rgba(2,6,23,.06); }
    .catmega-col:last-child{ border-right:0; }
    .catmega-title{ display:inline-block; margin:0 0 6px; font-weight:700; color:#111827; text-decoration:none; }
    .catmega-title:hover{ color:#c1126b; }
    .catmega-list{ list-style:none; margin:0; padding:0; }
    .catmega-link{ display:block; padding:6px 8px; border-radius:8px; color:#374151; text-decoration:none; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
    .catmega-link:hover{ background:rgba(193,18,107,.08); color:#c1126b; }

    @media (max-width:991.98px){
      .dropdown-menu.catmega{ width:100vw; left:50%!important; transform:translateX(-50%)!important; border-radius:0 0 14px 14px; }
      .catmega-inner{ padding:12px 14px; }
      .catmega-grid{ grid-template-columns:repeat(2,1fr); gap:12px 16px; }
      .catmega-col{ border-right:0; }
    }

    /* ===== Search overlay ===== */
    .search-overlay{ position:fixed; inset:0; display:flex; align-items:center; justify-content:center; background:rgba(255,255,255,.98); z-index:1055; opacity:0; visibility:hidden; transition:opacity .28s ease; }
    .search-overlay.active{ opacity:1; visibility:visible; }
    .search-overlay.search-overlay-dark{ background:rgba(0,0,0,.85); backdrop-filter:blur(5px); }
    .btn-close-search{ position:absolute; top:2rem; right:2rem; font-size:3rem; color:#333; background:none; border:0; line-height:1; }
    .search-overlay.search-overlay-dark .btn-close-search{ color:#fff; }
    .form-control-overlay{ width:min(92vw,600px); background:transparent; border:0; border-bottom:2px solid #ccc; font-size:2.25rem; text-align:center; padding:1rem 0; outline:none; box-shadow:none; color:#333; }
    .search-overlay.search-overlay-dark .form-control-overlay{ color:#fff; border-bottom-color:rgba(255,255,255,.55); }
    ...
    .search-overlay.search-overlay-dark .form-text-overlay, .search-overlay.search-overlay-dark .form-text-overlay a{ color:#fff; }
...
<body class="{{ request()->routeIs('home') ? 'is-home' : 'is-internal' }}">
  ...
  {{-- Search Overlay --}}
  <div id="search-overlay" class="search-overlay {{ request()->routeIs('home') ? 'search-overlay-dark' : '' }}" aria-hidden="true" role="dialog" aria-modal="true">
