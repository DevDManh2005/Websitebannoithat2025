...
<head>
    ...
    <style>
        ...
        *:focus-visible {
        ...
        body {
            overflow: hidden;
            /* Ngăn thanh cuộn của cả trang */
        }

        #wrapper {
            height: 100vh;
            /* Giới hạn chiều cao của wrapper bằng chiều cao màn hình */
        }

        /* 2. Cho phép sidebar tự cuộn khi nội dung quá dài */
        #sidebar-wrapper {
            position: static;
            /* Bỏ 'sticky' vì không còn cần thiết */
            height: 100vh;
            overflow-y: auto;
            /* Thêm thanh cuộn dọc cho sidebar khi cần */
            flex-shrink: 0;
            /* Ngăn sidebar bị co lại bởi flexbox */
        }
        ...
        main.content {
            flex-grow: 1;
            /* Cho phép main content lấp đầy không gian còn lại */
            overflow-y: auto;
            /* Thêm thanh cuộn riêng cho nội dung chính */
        }

        .app-navbar {
            flex-shrink: 0;
            /* Đảm bảo navbar trên cùng không bị co lại */
        }
        ...
        @media (max-width: 991.98px) {
            ...
            #wrapper.collapsed #sidebar-wrapper {
                transform: translateX(-100%);
                /* Khi ở mobile, trạng thái collapsed luôn bị ẩn */
            }
        ...
        @media (max-width: 767.98px) {
            ...
            .app-navbar .navbar-nav .nav-item .fw-600 {
                display: none;
                /* Ẩn tên người dùng, chỉ giữ lại avatar */
            }

            .app-navbar .d-flex.align-items-center.gap-2 {
                gap: 0.25rem !important;
                /* Giảm khoảng cách giữa nút sidebar và avatar */
            }
            ...
            .page-header {
                ...
                /* Các mục con chiếm toàn bộ chiều rộng */
                gap: 1rem;
                /* Tạo khoảng cách dọc */
            }
            ...
            .page-header>div {
                ...
                gap: 0.5rem;
                /* Thêm khoảng cách cho các nút bên trong */
            }

            /* --- Nội dung chính & Card --- */
            .content {
                padding: 0.75rem;
                /* Giảm padding hơn nữa trên mobile */
            }

            .card {
                border-radius: 12px;
                /* Giảm bo góc một chút cho gọn */
            }
        }
    </style>
        ...
        <aside id="sidebar-wrapper" aria-label="Sidebar">
            ...
            <nav class="nav-list">
                <a href="{{ route('admin.orders.index') }}"
                    class="nav-item {{ request()->routeIs('admin.orders.*') ? 'active' : '' }}"><i
                        class="bi bi-receipt"></i><span class="text">Đơn hàng</span></a>
                <a href="{{ route('admin.products.index') }}"
                    class="nav-item {{ request()->routeIs('admin.products.*') ? 'active' : '' }}"><i
                        class="bi bi-box-seam"></i><span class="text">Sản phẩm</span></a>
                <a href="{{ route('admin.reviews.index') }}"
                    class="nav-item {{ request()->routeIs('admin.reviews.*') ? 'active' : '' }}"><i
                        class="bi bi-chat-square-text"></i><span class="text">Đánh giá</span></a>
                <a href="{{ route('admin.categories.index') }}"
                    class="nav-item {{ request()->routeIs('admin.categories.*') ? 'active' : '' }}"><i
                        class="bi bi-layers"></i><span class="text">Danh mục</span></a>
                <a href="{{ route('admin.brands.index') }}"
                    class="nav-item {{ request()->routeIs('admin.brands.*') ? 'active' : '' }}"><i
                        class="bi bi-bag"></i><span class="text">Hãng</span></a>
                <a href="{{ route('admin.suppliers.index') }}"
                    class="nav-item {{ request()->routeIs('admin.suppliers.*') ? 'active' : '' }}"><i
                        class="bi bi-truck"></i><span class="text">Nhà cung cấp</span></a>
                <a href="{{ route('admin.inventories.index') }}"
                    class="nav-item {{ request()->routeIs('admin.inventories.*') ? 'active' : '' }}"><i
                        class="bi bi-archive"></i><span class="text">Kho</span></a>
                <a href="{{ route('admin.vouchers.index') }}"
                    class="nav-item {{ request()->routeIs('admin.vouchers.*') ? 'active' : '' }}"><i
                        class="bi bi-ticket-perforated"></i><span class="text">Voucher</span></a>
            </nav>

            <div class="nav-section-title">Nội dung</div>
            <nav class="nav-list">
                <a href="{{ route('admin.blog-categories.index') }}"
                    class="nav-item {{ request()->routeIs('admin.blog-categories.*') ? 'active' : '' }}"><i
                        class="bi bi-folder2-open"></i><span class="text">Danh mục bài viết</span></a>
                <a href="{{ route('admin.blogs.index') }}"
                    class="nav-item {{ request()->routeIs('admin.blogs.*') ? 'active' : '' }}"><i
                        class="bi bi-journal-text"></i><span class="text">Bài viết</span></a>
                <a href="{{ route('admin.slides.index') }}"
                    class="nav-item {{ request()->routeIs('admin.slides.*') ? 'active' : '' }}"><i
                        class="bi bi-images"></i><span class="text">Slides</span></a>
            </nav>
            ...
            <nav class="nav-list">
                @if (\Illuminate\Support\Facades\Route::has('admin.support_tickets.index'))
                    <a href="{{ route('admin.support_tickets.index') }}"
                        class="nav-item d-flex justify-content-between align-items-center {{ request()->routeIs('admin.support_tickets.*') ? 'active' : '' }}">
                        <span class="d-flex align-items-center gap-2"><i class="bi bi-life-preserver"></i><span
                                class="text">Hỗ trợ KH</span></span>
                        @php
                            try {
                                $eh_supportOpenCount = \App\Models\SupportTicket::where('status', 'open')->count();
                            } catch (\Throwable $e) {
                                $eh_supportOpenCount = null;
                            }
                        @endphp
            ...
            <nav class="nav-list pb-4">
                <a href="{{ route('admin.users.index') }}"
                    class="nav-item {{ request()->routeIs('admin.users.*') ? 'active' : '' }}"><i
                        class="bi bi-person-lines-fill"></i><span class="text">Người dùng</span></a>
                <a href="{{ route('admin.staffs.index') }}"
                    class="nav-item {{ request()->routeIs('admin.staffs.*') ? 'active' : '' }}"><i
                        class="bi bi-people"></i><span class="text">Nhân viên</span></a>
                <a href="{{ route('admin.admins.index') }}"
                    class="nav-item {{ request()->routeIs('admin.admins.*') ? 'active' : '' }}"><i
                        class="bi bi-person-badge"></i><span class="text">Admin</span></a>
                @if (\Illuminate\Support\Facades\Route::has('admin.permissions.index'))
                    <a href="{{ route('admin.permissions.index') }}"
                        class="nav-item {{ request()->routeIs('admin.permissions.*') ? 'active' : '' }}"><i
                            class="bi bi-shield-lock"></i><span class="text">Quyền</span></a>
                @endif
                <a href="{{ route('admin.route-permissions.index') }}"
                    class="nav-item {{ request()->routeIs('admin.route-permissions.*') ? 'active' : '' }}"><i
                        class="bi bi-diagram-3"></i><span class="text">Route → Quyền</span></a>
                <a href="{{ route('admin.settings.index') }}"
                    class="nav-item {{ request()->routeIs('admin.settings.*') ? 'active' : '' }}"><i
                        class="bi bi-gear-wide-connected"></i><span class="text">Cài đặt</span></a>
            </nav>
        </aside>
                    ...
                    <div class="flex-grow-1 px-lg-3 d-none d-md-block">
                        <div class="search-wrap mx-auto">
                            ...
                            <input id="quickSearch" class="form-control form-control-sm search-input"
                                placeholder="Tìm nhanh (Ctrl+/) – gõ: Đơn hàng, Sản phẩm, Báo cáo..."
                                autocomplete="off">
                    ...
                    <ul class="navbar-nav ms-auto align-items-center gap-2">
                        ...
                        <li class="nav-item dropdown">
                            ...
                            <ul class="dropdown-menu dropdown-menu-end shadow" aria-labelledby="notificationDropdown"
                                ...
                                <div id="notification-list" style="max-height: 400px; overflow-y: auto;">
                                    <li class="text-center text-muted p-3" id="no-notification-item">Không có thông
                                        báo mới.</li>
                                </div>
                            </ul>
                        </li>
                        ...
                        <li class="nav-item dropdown">
                            ...
                            <ul class="dropdown-menu dropdown-menu-end shadow" aria-labelledby="userMenu">
                                ...
                                <li><a class="dropdown-item" href="{{ route('admin.reports.dashboard') }}"><i
                                            class="bi bi-graph-up-arrow me-2"></i>Báo cáo</a></li>
                                <li>
                                    <hr class="dropdown-divider">
                                </li>
                                <li>
        ...
        <div class="modal-content"
            ...
            <div class="position-relative border-bottom">
                ...
                <input id="cmdInput" class="w-100"
                    ...
                    placeholder="Tìm lệnh / trang… (Đơn hàng, Sản phẩm, Báo cáo…)" autocomplete="off" />
                <span
...
@php
    ...
    $add = function (bool $ok, string $text, string $route, string $icon) use (&$q) {
        if ($ok) {
            $q[] = ['label' => $text, 'url' => route($route), 'icon' => $icon];
        }
    };
    $add(true, 'Bảng điều khiển', 'admin.dashboard', 'bi-speedometer2');
    $add(
        \Illuminate\Support\Facades\Route::has('admin.orders.index'),
        'Quản lý Đơn hàng',
        'admin.orders.index',
        'bi-receipt',
    );
    $add(
        \Illuminate\Support\Facades\Route::has('admin.products.index'),
        'Sản phẩm',
        'admin.products.index',
        'bi-box-seam',
    );
    $add(
        \Illuminate\Support\Facades\Route::has('admin.categories.index'),
        'Danh mục',
    ...
    $add(
        \Illuminate\Support\Facades\Route::has('admin.inventories.index'),
        'Kho',
        'admin.inventories.index',
        'bi-archive',
    );
    $add(
        \Illuminate\Support\Facades\Route::has('admin.vouchers.index'),
        'Voucher',
    ...
    $add(
        \Illuminate\Support\Facades\Route::has('admin.permissions.index'),
        'Quyền',
        'admin.permissions.index',
        'bi-shield-lock',
    );
    $add(
        \Illuminate\Support\Facades\Route::has('admin.route-permissions.index'),
        'Route ↔ Quyền',
        'admin.route-permissions.index',
        'bi-diagram-3',
    );
    $add(
        \Illuminate\Support\Facades\Route::has('admin.reports.dashboard'),
        'Báo cáo',
        'admin.reports.dashboard',
        'bi-graph-up-arrow',
    );
    $add(
        \Illuminate\Support\Facades\Route::has('admin.settings.index'),
        'Cài đặt',
