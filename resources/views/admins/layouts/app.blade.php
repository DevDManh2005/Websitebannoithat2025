...
<head>
    ...
    <style>
        ...
        @media (max-width: 991.98px) {
            #sidebar-wrapper { position: fixed; z-index: 1030; }
...
<div id="wrapper">
    ...
    <div class="border-end" id="sidebar-wrapper">
        ...
        <div class="list-group list-group-flush">
            ...
            {{-- Tùy chọn: chỉ hiển thị nếu dự án có route này --}}
            ...
            @if(\Illuminate\Support\Facades\Route::has('admin.support_tickets.index'))
                <a href="{{ route('admin.support_tickets.index') }}"
                   class="list-group-item list-group-item-action d-flex align-items-center justify-content-between p-3 {{ request()->routeIs('admin.support_tickets.*') ? 'active' : '' }}">
                    <span class="d-flex align-items-center">
                        <i class="bi bi-life-preserver me-2"></i> Hỗ trợ khách hàng
                    </span>
                    @php
                        try {
                    ...
                    @if(!is_null($eh_supportOpenCount) && $eh_supportOpenCount > 0)
                        <span class="badge text-bg-danger">{{ $eh_supportOpenCount }}</span>
                    @endif
                </a>
            @endif
