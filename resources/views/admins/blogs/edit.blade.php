{{-- resources/views/admins/blogs/edit.blade.php --}}
...
@php
  ...
  $thumbUrl = $thumb
    ? (\Illuminate\Support\Str::startsWith($thumb, ['http://','https://','//']) ? $thumb : \Illuminate\Support\Facades\Storage::url($thumb))
    : null;
@endphp
        ...
        <div class="card card-soft">
          ...
          <div class="card-body">
            ...
            <div class="mb-3">
              ...
              <textarea name="excerpt" rows="4" class="form-control">{{ old('excerpt', $blog->excerpt) }}</textarea>
            </div>

            <div class="mb-0">
              <label class="form-label">Nội dung <span class="text-danger">*</span></label>
              {{-- KHÔNG đặt required vì CKEditor sẽ ẩn phần tử này --}}
              <textarea name="content" id="editor" rows="10" class="form-control">{{ old('content', $blog->content) }}</textarea>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="col-lg-4">
          ...
          <div class="card-body">
            ...
            <select name="category_id" class="form-select">
              ...
              @foreach($categories as $c)
                <option value="{{ $c->id }}" {{ (string)old('category_id', $blog->category_id) === (string)$c->id ? 'selected' : '' }}>
                  {{ $c->name }}
                </option>
              @endforeach
            </select>
        ...
        <div class="card card-soft mb-3">
          ...
          <div class="card-body">
            ...
            @if($thumbUrl)
              <img id="thumbPreview" class="img-fluid rounded mt-2" src="{{ $thumbUrl }}" alt="preview">
            @else
...
<script>
  ...
  // Thumb preview (ưu tiên file mới)
