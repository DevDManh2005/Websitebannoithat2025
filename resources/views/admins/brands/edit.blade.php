{{-- resources/views/admins/brands/edit.blade.php --}}
...
<div class="container-fluid">
  ...
  <div class="d-flex justify-content-between align-items-center mb-3">
    <h1 class="h5 mb-0 fw-bold">Chỉnh sửa Thương hiệu</h1>
  ...
  <form action="{{ route('admin.brands.update', $brand) }}" method="POST" enctype="multipart/form-data" class="row g-3" novalidate>
    @csrf @method('PUT')
      ...
      <div class="card card-soft">
        ...
        <div class="card-body">
          ...
          <div class="mb-3">
            ...
            @if($logoUrl)
              <img src="{{ $logoUrl }}" alt="{{ $brand->name }}" class="img-fluid rounded" style="max-height:120px">
            @else
          ...
          <div class="row g-3">
            <div class="col-md-6">
              <label for="logo_file" class="form-label">Logo (Upload file mới)</label>
              <input type="file" name="logo_file" id="logo_file" class="form-control" accept="image/*">
              <div class="form-text">Nếu chọn file, hệ thống sẽ dùng file này.</div>
            </div>
            <div class="col-md-6">
              <label for="logo_url" class="form-label">Logo (URL mới)</label>
              <input type="url" name="logo_url" id="logo_url" class="form-control"
                     value="{{ old('logo_url') }}" placeholder="https://example.com/logo.png">
              <small class="form-text text-muted">Nếu không upload file, có thể nhập đường dẫn ảnh trực tiếp.</small>
            </div>
          </div>

          <div class="mt-3">
            <img id="logoPreview" class="img-fluid rounded {{ $logoUrl ? '' : 'd-none' }}" alt="Logo preview"
                 src="{{ $logoUrl ?: '' }}">
          </div>
        </div>
      </div>
