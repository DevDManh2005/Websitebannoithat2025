@php
    ...
    $resolveUrl = function ($routeVal) {
        if (!$routeVal) return '#';
        if (Route::has($routeVal)) return route($routeVal);
        if (Str::startsWith($routeVal, ['http://', 'https://', '/'])) return $routeVal;
        return url($routeVal);
    };
@endphp
<!DOCTYPE html>
...
<head>
  ...
  <style>
    ...
    *:focus-visible{ outline:2px solid var(--brand); outline-offset:2px }
    ...
    .sidebar-heading .brand-name{ color:#fff }
    ...
    .navbar-nav .nav-item{ overflow:visible !important; position:relative }
  ...
  <aside id="sidebar-wrapper" aria-label="Sidebar">
    ...
    <nav class="nav-list" role="navigation">
      @if(Route::has('staff.dashboard'))
        <a href="{{ route('staff.dashboard') }}" class="nav-item {{ request()->routeIs('staff.dashboard') ? 'active' : '' }}" data-bs-toggle="tooltip" title="Dashboard">
      ...
      <nav class="nav-list">
        @foreach($menu as $item)
          @php
            ...
            $active   = ($item['active'] ?? false) ? 'active' : (url()->current() === $href ? 'active' : '');
          @endphp
          <a class="nav-item {{ $active }}" href="{{ $href }}"><i class="bi {{ $icon }}"></i><span class="text">{{ $label }}</span></a>
        @endforeach
      </nav>
    @endif
      ...
      <div class="container-fluid py-2">
        ...
        <div class="flex-grow-1 px-lg-3 d-none d-md-block">
          <div class="search-wrap mx-auto">
            ...
            <input id="quickSearch" class="form-control form-control-sm search-input" placeholder="Tìm nhanh (Ctrl+/) – gõ: Bảng điều khiển, mục menu…" autocomplete="off">
  ...
  <div class="modal-dialog modal-dialog-centered" style="max-width:720px">
    <div class="modal-content" style="border-radius:16px; overflow:hidden; background:var(--card); box-shadow:0 30px 60px rgba(0,0,0,.25)">
      <div class="position-relative border-bottom">
        ...
        <input id="cmdInput" class="w-100" style="border:0; outline:none; padding:.9rem 2.75rem .9rem 2.25rem; background:transparent" placeholder="Tìm lệnh / trang… (Dashboard, Quyền, Mục menu…)" autocomplete="off" />
...
@php
  ...
  if (Route::has('staff.dashboard')) $push('Bảng điều khiển', route('staff.dashboard'), 'bi-speedometer2');
